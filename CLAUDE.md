# CLAUDE.md — Ajudante: Documentacao Oficial e Regras de Atuacao

> **LEIA TUDO ANTES DE TOCAR EM QUALQUER ARQUIVO.**
> Este documento e a unica fonte de verdade sobre a arquitetura, convencoes e regras do projeto.
...
├── CLAUDE.md                              # ESTE ARQUIVO — regras obrigatorias
├── .gitignore
│
├── src/
...
| engine    | runFlow, stopFlow, getStatus                        | JS → C#      |
| engine    | nodeStatusChanged, logMessage, flowCompleted, flowError | C# → JS  |
...
32. **Eventos do executor sao encaminhados automaticamente:** O `BridgeRouter` assina `FlowExecutor.NodeStatusChanged`, `LogMessage`, `FlowCompleted` e `FlowError` no construtor e envia os eventos correspondentes pelo canal `engine` para o frontend.
...
37. **Dados do usuario ficam em:** `%AppData%/Ajudante/flows/` (flows JSON), `%AppData%/Ajudante/logs/` (crash logs).
...
| `Bridge/BridgeRouter.cs` | `class BridgeRouter` | Despacha mensagens por canal. Handlers: save/load/list/new/deleteFlow, run/stop/getStatus, startMira/startSnip, getNodeDefinitions. Assina FlowExecutor events (NodeStatusChanged, LogMessage, FlowCompleted, FlowError) e envia para frontend via bridge. Gerencia overlays Mira/Snip. Evento: LogMessage |
...
| `MainWindow.xaml(.cs)` | `class MainWindow : Window` | Host principal. Inicializa NodeRegistry, FlowExecutor, WebBridge, BridgeRouter, SystemTrayManager. Encaminha eventos do executor para o frontend |
| `App.xaml(.cs)` | `class App : Application` | Mutex de instancia unica, cria diretorios AppData, exception handling global. Props estaticas: DataDirectory, FlowsDirectory, LogsDirectory |
...
4. BridgeRouter recebe → chama FlowExecutor.ExecuteAsync()
5. FlowExecutor encontra o node de entrada (sem conexoes de entrada)
...
Para triggers:
```
1. TriggerManager.ActivateFlowTriggersAsync(flow)
2. Cada ITriggerNode chama StartWatchingAsync()
3. Quando evento ocorre → trigger.Triggered dispara
4. TriggerManager chama FlowExecutor.ExecuteFromTriggerAsync()
5. Execucao continua a partir do trigger node
```
